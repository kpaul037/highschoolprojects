...
public class StarChart {
    ...
    public void starsAndMagnitudePlotter(HashTable coord, HashTable mag, ArrayList<Integer> list) {
        StdDraw.setPenColor(StdDraw.WHITE);
        double StarSize = 0;
        for (Integer i : list) {
            Coordinate temp = translate((Coordinate) coord.get(i));
            StarSize = Math.round(10.0 / ((double) mag.get(i) + 2.0));
            StdDraw.filledCircle(temp.x, temp.y, StarSize);
        }
    }

    public void constellationsPlotter(HashTable coordinates, ArrayList<Couple> list) {
        StdDraw.setPenColor(StdDraw.WHITE);
    ...
    public HashTable[] readCoords(String fileName) throws IOException {
        ...
        HashTable coordinates = new HashTable(10133);
        HashTable magnitude = new HashTable(10133);
        HashTable names = new HashTable(10133);
    ...
    public ArrayList<Integer> getHDList() { return hD; }
}